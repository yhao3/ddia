...
2000 年以後，非關係（NoSQL）資料庫開始普及。它們的目標是在關係資料庫的現狀基礎上，透過提供新的資料模型選擇（請參閱 [第二章](./ch2)）並預設包含複製（第五章）和分區（第六章）來進一步提升。交易是這次運動的主要犧牲品：這些新一代資料庫中的許多資料庫完全放棄了交易，或者重新定義了這個詞，描述比以前所理解的更弱得多的一套保證[^ref_4]。
...
* [一致性雜湊](./ch6#一致性雜湊) 是某些系統用於重新分區的一種分區方法。
...
> * 一個相關性故障（停電，或一個特定輸入導致所有節點崩潰的 Bug）可能會一次性摧毀所有副本（請參閱「[可靠性](./ch1#可靠性)」），任何僅儲存在記憶體中的資料都會遺失，故記憶體資料庫仍然要和磁碟寫入打交道。
> * 在非同步複製系統中，當主庫不可用時，最近的寫入操作可能會遺失（請參閱「[處理節點宕機](./ch5#處理節點宕機)」）。
...
這些問題非常讓人頭大，故儲存引擎一個幾乎普遍的目標是：對單節點上的單個物件（例如鍵值對）上提供原子性和隔離性。原子性可以透過使用日誌來實現崩潰恢復（請參閱 “[讓 B 樹更可靠](./ch3#讓B樹更可靠)”），並且可以使用每個物件上的鎖來實現隔離（每次只允許一個執行緒訪問物件） 。
...
許多分散式資料儲存已經放棄了多物件交易，因為多物件交易很難跨分區實現，而且在需要高可用性或高效能的情況下，它們可能會礙事。但說到底，在分散式資料庫中實現交易，並沒有什麼根本性的障礙。[第九章](./ch9) 將討論分散式交易的實現。
...
- 但是，讀已提交並不能防止 [圖 7-1](./v1/ddia_0701.png) 中兩個計數器增量之間的競爭狀態。在這種情況下，第二次寫入發生在第一個交易提交後，所以它不是一個髒寫。這仍然是不正確的，但是出於不同的原因，在 “[防止遺失更新](#防止遺失更新)” 中將討論如何使這種計數器增量安全。
...
為了實現快照隔離，資料庫使用了我們看到的用於防止 [圖 7-4](./v1/ddia_0704.png) 中的髒讀的機制的一般化。資料庫必須可能保留一個物件的幾個不同的提交版本，因為各種正在進行的交易可能需要看到資料庫在不同的時間點的狀態。因為它同時維護著單個物件的多個版本，所以這種技術被稱為 **多版本併發控制（MVCC, multi-version concurrency control）**。
...
在 CouchDB、Datomic 和 LMDB 中使用另一種方法。雖然它們也使用 [B 樹](./ch3#B樹)，但它們使用的是一種 **僅追加 / 寫時複製（append-only/copy-on-write）** 的變體，它們在更新時不覆蓋樹的頁面，而為每個修改頁面建立一份副本。從父頁面直到樹根都會連動（Cascade）更新，以指向它們子頁面的新版本。任何不受寫入影響的頁面都不需要被複製，並且保持不變[^ref_33] [^ref_34] [^ref_35]。
...
併發的寫入交易之間還有其他幾種有趣的衝突。其中最著名的是 **遺失更新（lost update）** 問題，如 [圖 7-1](./v1/ddia_0701.png) 所示，以兩個併發計數器增量為例。
...
例如，考慮一個多人遊戲，其中幾個玩家可以同時移動相同的棋子。在這種情況下，一個原子操作可能是不夠的，因為應用程式還需要確保玩家的移動符合遊戲規則，這可能涉及到一些不能合理地用資料庫查詢實現的邏輯。但你可以使用鎖來防止兩名玩家同時移動相同的棋子，如 [例 7-1](#例-7-1-顯式鎖定行以防止遺失更新) 所示。
...
在複製資料庫中（請參閱 [第五章](./ch5)），防止遺失的更新需要考慮另一個維度：由於在多個節點上存在資料副本，並且在不同節點上的資料可能被併發地修改，因此需要採取一些額外的步驟來防止遺失更新。
...
相反，如 “[檢測併發寫入](./ch5#檢測併發寫入)” 一節所述，這種複製資料庫中的一種常見方法是允許併發寫入建立多個衝突版本的值（也稱為兄弟），並使用應用程式碼或特殊資料結構在事實發生之後解決和合併這些版本。
...
另一方面，最後寫入勝利（LWW）的衝突解決方法很容易遺失更新，如 “[最後寫入勝利（丟棄併發寫入）](./ch5#最後寫入勝利（丟棄併發寫入）)” 中所述。不幸的是，LWW 是許多複製資料庫中的預設方案。
...
- **多人遊戲**：在 [例 7-1](#例-7-1-顯式鎖定行以防止遺失更新) 中，我們使用一個鎖來防止遺失更新（也就是確保兩個玩家不能同時移動同一個棋子）。但是鎖定並不妨礙玩家將兩個不同的棋子移動到棋盤上的相同位置，或者採取其他違反遊戲規則的行為。取決於你正在執行的規則型別，也許可以使用唯一約束（unique constraint），否則你很容易發生寫入偏差。
...
- 資料庫設計人員意識到 OLTP 交易通常很短，而且只進行少量的讀寫操作（請參閱 “[交易處理還是分析？](./ch3#交易處理還是分析？)”）。相比之下，長時間執行的分析查詢通常是唯讀的，因此它們可以在序列執行迴圈之外的一致快照（使用快照隔離）上執行。
...
出於這個原因，具有單執行緒序列交易處理的系統不允許互動式的多語句交易。取而代之，應用程式必須==提前將整個交易程式碼作為預存程序提交給資料庫==。這些方法之間的差異如 [圖 7-9](./v1/ddia_0709.png) 所示。如果交易所需的所有資料都在記憶體中，則預存程序可以非常快地執行，而不用等待任何網路或磁碟 I/O。
...
為了伸縮至多個 CPU 核心和多個節點，可以對資料進行分區（請參閱 [第六章](./ch6)），在 VoltDB 中支援這樣做。如果你可以找到一種對資料集進行分區的方法，以便每個交易只需要在單個分區中讀寫資料，那麼每個分區就可以擁有自己獨立執行的交易處理執行緒。在這種情況下可以為每個分區指派一個獨立的 CPU 核，交易吞吐量就可以與 CPU 核數保持線性伸縮[^ref_47]。
...
交易是否可以是劃分至單個分區很大程度上取決於應用資料的結構。簡單的鍵值資料通常可以非常容易地進行分區，但是具有多個次級索引的資料可能需要大量的跨分區協調（請參閱 “[分區與次級索引](./ch6#分區與次級索引)”）。
...
顧名思義，SSI 基於快照隔離 —— 也就是說，交易中的所有讀取都是來自資料庫的一致性快照（請參閱 “[快照隔離和可重複讀取](#快照隔離和可重複讀)”）。與早期的樂觀併發控制技術相比這是主要的區別。在快照隔離的基礎上，SSI 添加了一種演算法來檢測寫入之間的序列化衝突，並確定要中止哪些交易。
...
回想一下，快照隔離通常是透過多版本併發控制（MVCC；見 [圖 7-10](./v1/ddia_0710.png)）來實現的。當一個交易從 MVCC 資料庫中的一致快照讀時，它將忽略取快照時尚未提交的任何其他交易所做的寫入。在 [圖 7-10](./v1/ddia_0710.png) 中，交易 43 認為 Alice 的 `on_call = true` ，因為交易 42（修改 Alice 的待命狀態）未被提交。然而，在交易 43 想要提交時，交易 42 已經提交。這意味著在讀一致性快照時被忽略的寫入已經生效，交易 43 的前提不再為真。
...
第二種情況要考慮的是另一個交易在讀取資料之後修改資料。這種情況如 [圖 7-11](./v1/ddia_0711.png) 所示。
...
在 [圖 7-11](./v1/ddia_0711.png) 中，交易 42 和 43 都在班次 1234 查詢值班醫生。如果在 `shift_id` 上有索引，則資料庫可以使用索引項 1234 來記錄交易 42 和 43 讀取這個資料的事實。（如果沒有索引，這個資訊可以在表級別進行追蹤）。這個資訊只需要保留一段時間：在一個交易完成（提交或中止），並且所有的併發交易完成之後，資料庫就可以忘記它讀取的資料了。

當交易寫入資料庫時，它必須在索引中查詢最近曾讀取受影響資料的其他交易。這個過程類似於在受影響的鍵範圍上獲取寫鎖，但鎖並不會阻塞交易直到其他讀交易完成，而是像警戒線一樣只是簡單通知其他交易：你們讀過的資料可能不是最新的啦。

在 [圖 7-11](./v1/ddia_0711.png) 中，交易 43 通知交易 42 其先前讀已過時，反之亦然。交易 42 首先提交並成功，儘管交易 43 的寫影響了 42 ，但因為交易 43 尚未提交，所以寫入尚未生效。然而當交易 43 想要提交時，來自交易 42 的衝突寫入已經被提交，所以交易 43 必須中止。
