...
但是，對於需要訪問多個分割槽的任何交易，資料庫必須在觸及的所有分割槽之間協調交易。預存程序需要跨越所有分割槽鎖定執行，以確保整個系統的可序列性。
...
從某種意義上說，序列執行可以稱為悲觀到了極致：在交易持續期間，每個交易對整個資料庫（或資料庫的一個分割槽）具有排它鎖，作為對悲觀的補償，我們讓每筆交易執行得非常快，所以只需要短時間持有 “鎖”。
...
為了防止這種異常，資料庫需要跟蹤一個交易由於 MVCC 可見性規則而忽略另一個交易的寫入。當交易想要提交時，資料庫檢查是否有任何被忽略的寫入現在已經被提交。如果是這樣，交易必須中止。
...
在 [圖 7-11](./v1/ddia_0711.png) 中，交易 42 和 43 都在班次 1234 查詢值班醫生。如果在 `shift_id` 上有索引，則資料庫可以使用索引項 1234 來記錄交易 42 和 43 讀取這個資料的事實。（如果沒有索引，這個資訊可以在表級別進行跟蹤）。這個資訊只需要保留一段時間：在一個交易完成（提交或中止），並且所有的併發交易完成之後，資料庫就可以忘記它讀取的資料了。
...
與往常一樣，許多工程細節會影響演算法的實際表現。例如一個權衡是跟蹤交易的讀取和寫入的 **粒度（granularity）**。如果資料庫詳細地跟蹤每個交易的活動（細粒度），那麼可以準確地確定哪些交易需要中止，但是簿記開銷可能變得很顯著。簡略的跟蹤速度更快（粗粒度），但可能會導致更多不必要的交易中止。
...
與序列執行相比，可序列化快照隔離並不侷限於單個 CPU 核的吞吐量：FoundationDB 將序列化衝突的檢測分佈在多臺機器上，允許擴充套件到很高的吞吐量。即使資料可能跨多臺機器進行分割槽，交易也可以在保證可序列化隔離等級的同時讀寫多個分割槽中的資料[^ref_54]。
