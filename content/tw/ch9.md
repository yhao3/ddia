...
* *狀態機複製*，我們將在 ["使用共享日誌"](/tw/ch10#sec_consistency_smr) 中討論，透過在每個副本上獨立執行相同的確定性事務序列來複制資料。我們已經看到了這個想法的兩個變體：基於語句的複製（見 ["複製日誌的實現"](/tw/ch6#sec_replication_implementation)）和使用預存程序的序列事務執行（見 ["預存程序的利弊"](/tw/ch8#sec_transactions_stored_proc_tradeoffs)）。
