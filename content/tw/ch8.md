...
如果兩個交易不訪問相同的資料，或者都是隻讀的，它們可以安全地並行執行，因為它們互不依賴。僅當一個交易讀取另一個交易併發修改的資料時，或者當兩個交易嘗試同時修改相同的資料時，才會出現併發問題（競態條件）。
...
* 資料庫設計者意識到 OLTP 交易通常很短，只進行少量讀寫（參見["分析與運營系統"](/tw/ch1#sec_introduction_analytics)）。相比之下，長時間執行的分析查詢通常是隻讀的，因此它們可以在序列執行迴圈之外的一致快照上執行（使用快照隔離）。
...
VoltDB 還使用預存程序進行復制：它不是將交易的寫入從一個節點複製到另一個節點，而是在每個副本上執行相同的預存程序。因此，VoltDB 要求預存程序是*確定性的*（在不同節點上執行時，它們必須產生相同的結果）。例如，如果交易需要使用當前日期和時間，它必須透過特殊的確定性 API 來實現（有關確定性操作的更多詳細資訊，請參見["持久執行和工作流"](/tw/ch5#sec_encoding_dataflow_workflows)）。這種方法稱為*狀態機複製*，我們將在[第 10 章](/tw/ch10#ch_consistency)中回到它。
...
為什麼要等到提交？為什麼不在檢測到陳舊讀取時立即中止交易 43？好吧，如果交易 43 是隻讀交易，它就不需要被中止，因為沒有寫偏斜的風險。在交易 43 進行讀取時，資料庫還不知道該交易是否稍後會執行寫入。此外，交易 42 可能還會中止，或者在交易 43 提交時可能仍未提交，因此讀取可能最終不是陳舊的。透過避免不必要的中止，SSI 保留了快照隔離對從一致快照進行長時間執行讀取的支援。
