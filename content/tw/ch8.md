...
為了防止這種異常，資料庫需要跟蹤交易由於 MVCC 可見性規則而忽略另一個交易的寫入的時間。當交易想要提交時，資料庫會檢查是否有任何被忽略的寫入現在已經提交。如果是，交易必須被中止。
...
在[圖 8-11](/tw/ch8#fig_transactions_detect_index_range) 中，交易 42 和 43 都在班次 `1234` 期間搜尋值班醫生。如果 `shift_id` 上有索引，資料庫可以使用索引條目 1234 來記錄交易 42 和 43 讀取此資料的事實。（如果沒有索引，可以在表級別跟蹤此資訊。）此資訊只需要保留一段時間：在交易完成（提交或中止）並且所有併發交易完成後，資料庫可以忘記它讀取的資料。
...
與往常一樣，許多工程細節會影響演算法在實踐中的工作效果。例如，一個權衡是跟蹤交易讀寫的粒度。如果資料庫詳細跟蹤每個交易的活動，它可以精確地確定哪些交易需要中止，但簿記開銷可能變得很大。不太詳細的跟蹤速度更快，但可能導致比嚴格必要更多的交易被中止。
...
使用 2PC 時，分散式交易從應用程式在多個數據庫節點上正常讀寫資料開始。我們稱這些資料庫節點為交易中的*參與者*。當應用程式準備提交時，協調器開始第 1 階段：它向每個節點發送*準備*請求，詢問它們是否能夠提交。然後協調器跟蹤參與者的響應：
...
交易協調器實現 XA API。該標準沒有指定應該如何實現它，但在實踐中，協調器通常只是載入到發出交易的應用程式的同一程序中的庫（而不是單獨的服務）。它跟蹤交易中的參與者，在要求他們準備後收集參與者的響應（透過驅動程式的回撥），並使用本地磁碟上的日誌來跟蹤每個交易的提交/中止決定。
